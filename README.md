...
- 每次上传到 GitHub 主仓库，视为发布一个新的小版本（Patch）。
- 版本号建议使用 `v1.0.x` 形式（例如 `v1.0.14`）。
- 仓库只提交可公开发布内容：源代码、资源文件（图标与词库）、文档、许可证与声明文件、以及打包脚本。
- 调试和构建产物（如 `build/`、`Debug/`、`Release/`、日志、符号文件）不提交。
- 可安装 MSI 需放在仓库根目录，作为该版本的发布工件。

可用脚本（自动把最新 MSI 放到仓库根目录并按版本重命名）：

```powershell
./scripts/prepare_github_patch_release.ps1 -Version 1.0.14
```
